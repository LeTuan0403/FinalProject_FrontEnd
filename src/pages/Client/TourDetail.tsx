...
const TourDetail = () => {
  ...
  return (
    <>
      <div className="bg-gray-50 pb-32 pt-10">
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden relative">
                  ...
                  <div className="p-5 bg-slate-50 border-b border-gray-100 text-center">
                    <p className="text-gray-500 text-xs font-bold mb-1 uppercase tracking-widest">Giá trọn gói</p>
                    {(() => {
                      ...
                      if (tour.ngayKhoiHanh && tour.discounts) {
                        ...
                        if (sortedDates.length > 0) {
                          const nextDateStr = getLocalDateStr(sortedDates[0]);
                          const discount = tour.discounts.find(d => getLocalDateStr(new Date(d.date)) === nextDateStr);
                          if (discount) {
                      ...
                      return (
                        <div className="flex flex-col items-center">
                          <div className="flex items-baseline gap-2">
                            {discountPercent > 0 && <span className="text-gray-400 line-through text-base font-medium">{originalPrice.toLocaleString()}đ</span>}
                            <p className="text-3xl font-black text-red-600">
                              {displayPrice.toLocaleString()}
                              <span className="text-sm text-gray-500 font-normal"> đ/khách</span>
                            </p>
                          </div>
                          {discountPercent > 0 && (
                  ...
                  <div className="p-6 space-y-5">
                    ...
                    <button
                      onClick={() => {
                        window.dispatchEvent(new CustomEvent('open_chat_with_tour', { detail: { tour } }));
                      }}
      ...
      < div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 z-[1000] shadow-[0_-4px_25px_rgba(0,0,0,0.15)] pb-[max(0.75rem,env(safe-area-inset-bottom))]" >
        <div className="flex items-center justify-between gap-3 w-full">
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-[10px] text-gray-500 uppercase font-bold tracking-wider leading-none mb-1">Giá trọn gói</span>
            {(() => {
              const todayStr = getLocalDateStr(new Date());
              let displayPrice = (tour.isTuChon ? Math.round(tour.tongGiaDuKien / (tour.soLuongCho || 1)) : tour.tongGiaDuKien);
              let discountPercent = 0;
              ...
              return (
                <div className="flex flex-col leading-none">
                  <span className="text-xl font-black text-red-600 truncate">
                    {displayPrice.toLocaleString()}đ
                    <span className="text-[10px] text-gray-500 font-normal lowercase ml-1">/khách</span>
                  </span>
                  {discountPercent > 0 && (
                    <div className="mt-1">
                      <span className="text-[10px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full font-bold">-{discountPercent}%</span>
                    </div>
                  )}
                </div>
              );
          ...
          <button
            onClick={() => navigate(`/booking/${tour.tourId}`)}
            className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold h-12 px-2 rounded-xl transition shadow-lg shadow-blue-200 uppercase text-sm tracking-tight flex items-center justify-center gap-1.5 shrink-0 whitespace-nowrap active:scale-95 duration-150"
          >
