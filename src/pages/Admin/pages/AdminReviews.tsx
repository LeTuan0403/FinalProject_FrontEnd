            ...
            {filteredReviews.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-6">
                    ...
                    {filteredReviews.map((rv) => {
                        ...
                        const userName = isAnonymous ? 'Người dùng ẩn danh' : (rv.nguoiDung?.hoTen || (typeof rv.userId === 'object' ? (rv.userId as unknown as { hoTen: string }).hoTen : `User #${rv.userId}`));
                        const tourName = rv.tour?.tenTour || (typeof rv.tourId === 'object' ? (rv.tourId as unknown as { tenTour: string }).tenTour : `Tour #${rv.tourId}`);
                                                    ...
                                                    <div className="space-y-3 mb-4 max-h-[300px] overflow-y-auto pr-2">
                                                        {rv.replies && rv.replies.length > 0 ? (
                                                            rv.replies.map((reply, rIdx) => {
                                                                const rUser = typeof reply.userId === 'object' ? reply.userId as unknown as { isAdmin?: boolean; role?: string | number; hoTen: string; email: string } : null;
